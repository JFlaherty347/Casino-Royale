...
class BlackjackEnvDealer(py_environment.PyEnvironment):
   ...
   #Resets Environment to initial state. Returns state of environment
    ...
    def _step(self, action):
        ...
        if (self._episode_ended or self._state[0] >= 21):
            #check if bust, set score accordingly
            if(self._state[0] > 21):
                self._state[0] = -21 #-21 -> bust

            #reward = 1 if win, 0 if tie, -1 if lose
            resultReward = 0
            dealerScore = self.getDealerScore()
            if(self._state[0] > dealerScore):
                resultReward = 1
            elif(self._state[0] < dealerScore):
                resultReward = -1
    ...
    #helper method to calculate a dealer's score (same state layout as agent)
